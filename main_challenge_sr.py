...
#    print(torch.backends.cudnn.version())  # cudnn version
    ...
    # basic settings
    # --------------------------------
    model_names = ['msrresnet', 'imdn' , 'dncnn' , 'ffdnet' , 'unet' , 'unet2']
    model_id = 5                  # set the model name
    sf = 4
    ...
    if model_name == 'msrresnet':
        ...
        model = net(in_nc=3,
                   ...
                   act_mode="BR")
        model_path = os.path.join('model_zoo', 'dncnn.pth')            # set model path
        ...
        model = net(in_nc=3,
                   ...
                   nb=15,
                   act_mode="R")
        model_path = os.path.join('model_zoo', 'ffdnet.pth')            # set model path
    elif model_name == 'unet':
        from models.network_dncnn import UNet as net
        model = net()
        model_path = os.path.join('model_zoo', 'unet.pth')            # set model path
    elif model_name == 'unet2':
        from models.network_dncnn import UNet2 as net
        model = net()
        model_path = os.path.join('model_zoo', 'unet2.pth')            # set model path
