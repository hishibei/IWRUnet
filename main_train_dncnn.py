...
def main(json_path='options/train_dncnn.json'):
    ...
    for epoch in range(1000000):  # keep running
        for i, train_data in enumerate(train_loader):
            ...
            # 6) testing
            # -------------------------------
            if current_step % opt['train']['checkpoint_test'] == 0 and epoch <= 100:
                ...
                for test_data in test_loader:
                    ...
                    # 读取原图片尺寸（用于将图片尺寸还原）
                    # img_dir_ori = os.path.join(opt['datasets']['test']['dataroot_H'], image_name_ext)
                    # img = Image.open(img_dir_ori)
                    # w , h = img.size
                    ...
                    # 将图片组恢复为原图片大小
                    # E_img = cv2.resize(E_img,(w,h))
                    # H_img = cv2.resize(H_img,(w,h))
                    # L_img = cv2.resize(L_img,(w,h))
                    ...
                    save_img_path = os.path.join(img_dir, '{:s}_{:d}_L_img.png'.format(img_name, epoch))
                    util.imsave(L_img, save_img_path)

                    save_img_path = os.path.join(img_dir, '{:s}_{:d}_H_img.png'.format(img_name, epoch))
                ...
                logger.info('<epoch:{:3d}, iter:{:8,d}, Average SSIM : {:<.4f}\n'.format(epoch, current_step, avg_ssim))
